...
#해외센터 지역 코로나 현황 Release 2차 
# v1.1 Webdriver 스크롤 적용   
...
#스크롤 높이 srollTo(12,480)
#특정위치 
browser.execute_script("windows.scorollTo(478,480);")
point = browser.find_element_by_css_selector(CSS)[-1]
browser.execute_script("arguments[0].scrollintoView(true);",point);
last_page_height = driver.execute_script("return document.documentElement.scrollHeight") 

while True: 
    driver.execute_script("window.scrollTo(0, document.documentElement.scrollHeight);") 
    #driver.execute_script("window.scrollTo(12,480);")
    time.sleep(3.0) 
...
df = pd.DataFrame(result, columns=['국가','확진자수','백만명당','완치자수', '사망자수'])
...
# df2.to_excel(excel_writer=r"K:/My files/Download/DC_COVID19_2/{0}".format('covid19_daily' + datestring + '.xlsx'),\
